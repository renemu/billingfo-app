...
const categories = [
  { name: "Software", count: 5, total: "Rp 1.200.000", color: "bg-emerald-500" },
...
const upcomingPayments = [
  { name: "Netflix", amount: "Rp 54.000", dueDate: "10 Feb 2026", category: "Subscription" },
  { name: "GitHub Pro", amount: "Rp 84.000", dueDate: "12 Feb 2026", category: "Software" },
  { name: "AWS", amount: "Rp 320.000", dueDate: "15 Feb 2026", category: "Cloud" },
  { name: "Spotify", amount: "Rp 54.000", dueDate: "18 Feb 2026", category: "Subscription" },
  { name: "Figma", amount: "Rp 145.000", dueDate: "20 Feb 2026", category: "Software" },
];
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat, index) => (
          ...
          >
            <div className={`mb-4 inline-flex rounded-xl p-2.5 ${stat.bgColor} text-slate-700 dark:text-slate-300`}>
              {stat.icon}
            </div>
              ...
              >
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 sm:h-10 sm:w-10">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-slate-500 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                    </svg>
                  </div>
                  <div className="min-w-0">
                    <p className="truncate font-medium text-slate-900 dark:text-white">{payment.name}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400 sm:text-sm">{payment.category}</p>
                  </div>
                </div>
                <div className="flex items-center justify-between sm:justify-end sm:text-right gap-2 shrink-0">
                  <p className="font-semibold text-slate-900 dark:text-white text-sm sm:text-base">{payment.amount}</p>
                  <p className="text-xs text-amber-600 dark:text-amber-400 sm:text-sm">{payment.dueDate}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between gap-2 text-sm min-w-0">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="shrink-0 text-slate-500 dark:text-slate-400">({category.count})</span>
                  </div>
                  <span className="shrink-0 font-medium text-slate-900 dark:text-white">{category.total}</span>
                </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm sm:p-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:gap-4">
          ...
          >
            ...
            <span className="font-medium text-slate-900 dark:text-white">Tambah Langganan Baru</span>
          </Link>
          <Link
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-500/10">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-amber-600 dark:text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
            </div>
            <span className="font-medium text-slate-900 dark:text-white">Atur Pengingat</span>
          </Link>
          <Link
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-violet-500/10">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-violet-600 dark:text-violet-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
              </svg>
            </div>
            <span className="font-medium text-slate-900 dark:text-white">Tambah Kategori</span>
          </Link>
        </div>
      </div>
    </div>
